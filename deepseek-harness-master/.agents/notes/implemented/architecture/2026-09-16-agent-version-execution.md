...
Run admission fixes the version and persists its identity before invoking the existing Session Controller and Harness Loop. One Run owns one Session and one submitted task. The Session log carries `platform/run` attribution alongside the original execution events; queries derive completion from durable turn endings, not the current deployment. Accepted requests are never automatically resubmitted after process loss.
