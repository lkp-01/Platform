...
可选的 `selection` 固定远端工具名称、描述、输入/输出 Schema 和可选的公开别名。省略时桥接全部发现工具；显式空数组不桥接任何工具。初始同步、通知和重连都在注册前应用选择。选中项的描述必须匹配；缺失或变化会撤销原有注册并使更新失败。未选中工具的增加不会扩大模型工具集合。这些严格语义仅对指定选择的桥接取代普通的保留最近有效列表行为。作用域注册保持隔离；仅使用 `tools.restrict()` 只能屏蔽继承的全局工具。

`discoverMcpServer()` 返回描述，不产生注册表副作用。`prepareMcpTools()` 为受管理的调用方提供原生定义，每次操作使用新凭证并在串行的短连接中执行，复用插件的传输、Schema 校验和输出投影。调用方将返回定义注册到 Agent 作用域并提供生命周期取消信号。发现和调用均有时限；返回结果前关闭连接。HTTP 拒绝重定向以防止转发凭证。
...
- **要么完整世代，要么没有。** 同步会原子地交换世代：未指定选择时，获取失败保留上一世代，注册冲突则回滚整个尝试中的世代。
...
监督器监听 `notifications/tools/list_changed` 并排队一次重新同步；未指定选择时，获取阶段失败会保留上一世代注册，注册冲突则回滚本次尝试的世代。每次中断共享一个尝试预算：连续失败达到 `maxAttempts` 次后工具被注销、重连停止；连接存活超过 `maxDelayMs` 会重置预算。
