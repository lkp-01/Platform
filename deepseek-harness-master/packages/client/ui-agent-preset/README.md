...
With the business authoring service mounted, the **Agents** sidebar panel lists durable resources and provides creation, draft editing, archival and restoration. Detail links use `#agents/<id>` and survive reload. Organization and owner are explicitly shared Host configuration. Versions provide explicit snapshot saving, read-only configuration, deployment and rollback; the overview separates latest saved and currently deployed versions. Runs submit tasks to the deployed version and show real execution history with links to configuration and conversation records. Version-bound conversations replace input and model controls with a read-only explanation and active-task cancellation. The separate **Agent library** retains immutable Preset creation for legacy conversations; those definitions also appear in the resource Registry.
