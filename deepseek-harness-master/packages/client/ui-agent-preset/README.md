...
With the business authoring service mounted, the **Agents** sidebar panel lists durable resources and provides creation, draft editing, archival and restoration. Detail links use `#agents/<id>` and survive reload. Organization and owner are explicitly shared Host configuration. Versions provide explicit snapshot saving, read-only configuration, deployment and rollback; the overview separates latest saved and currently deployed versions. Runs submit tasks to the deployed version and show five-state execution history, version/status filters and direct details at `#agents/<id>/runs/<runId>`. Details include actor, timing, input, final output or error, plus configuration and conversation links. Active rows poll until terminal; interruption timing is marked unknown. Cancellation stays pending until Harness confirms settlement. Version-bound conversations replace input and model controls with a read-only explanation and active-task cancellation. The separate **Agent library** retains immutable Preset creation for legacy conversations; those definitions also appear in the resource Registry.

Mount this plugin alongside the settings and conversation packages; the management section then shows a visibility switch that is on by default. While it is off, the new-session chip is absent and the Host composes an unnamed session from the deployment default (`standard` in the shipped Web bundle). Turning it on restores the saved user default, or uses the deployment default when none has been saved, and carries that default to the current blank task; a chip pick creates a separate Session with the requested preset and preserves the current Workspace. The Start a new chat button creates another Session with the displayed preset. Creation failures remain visible beside the picker and keep the composer blocked until a successful retry. Turning the picker off again returns the current blank task to the deployment default the same way and discards an unconsumed stage; started and historical sessions keep their labels, compositions, and recorded history.
