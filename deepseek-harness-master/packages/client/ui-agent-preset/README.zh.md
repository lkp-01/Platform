...
挂载业务创建服务后，侧栏 **Agents** 页面列出持久化资源，支持创建、草稿编辑、归档与恢复。详情链接使用 `#agents/<id>`，刷新后仍可打开。组织空间与归属团队明确来自共享 Host 配置。版本页提供显式保存快照、只读配置、部署与回滚；概览区分最新保存版本和当前部署版本。Run 向已部署版本提交任务，展示五态执行历史、版本/状态筛选及 `#agents/<id>/runs/<runId>` 直达详情。详情包含发起者、时间、输入、最终输出或错误，以及配置和会话链接。活跃记录轮询至终态；中断执行的耗时标为未知。取消请求在 Harness 确认停止前保持处理中。关联版本的会话以只读说明和正在执行任务的取消按钮替代输入与模型控件。独立的 **Agent 工作台**保留旧式会话的不可变 Preset 创建入口；这些定义也会出现在资源 Registry 中。
...
挂载 AgentBuilder 服务后，**资源中心**提供注册、草稿编辑、版本发布、生命周期控制和引用方链接。编辑 Agent 时选择已发布的 Model、Tool 和 Skill 明确版本。版本详情展示固定资源清单，发布资源不会修改既有选择。所有资源操作使用 demo 的共享 Host。

Run 详情显示恢复状态、执行轮次、checkpoint、重试时间和截止时间。BLOCKED 任务提供**记录核实结果**入口，填写工具调用 ID 与核实依据。服务端先保存决定，再继续执行；阻塞或等待期间仍可取消。页面复用现有 Run 与 Trace API。
...
Run 详情包含 **Trace** 时间线，展示模型/工具调用、结果、错误与最终答案，以及提供方返回的 Token、调用次数、有界可展开预览和缺失数据提示。任务结束后 Trace 仍独立轮询，直到投影结算；部分数据或不可用状态提供刷新操作。分页从同一版本重新加载可见页面，失效游标显示可重试错误。Run 详情保留原始执行入口。全部 Trace 文案使用 Agent Registry 本地化字典。

Agent 详情提供运行分析页签，支持运行样本筛选、版本对比、依赖健康与 Trace 下钻。缺失指标展示数据覆盖；运行成功率与答案质量分开。
