...
挂载业务创建服务后，侧栏 **Agents** 页面列出持久化资源，支持创建、草稿编辑、归档与恢复。详情链接使用 `#agents/<id>`，刷新后仍可打开。组织空间与归属团队明确来自共享 Host 配置。版本页提供显式保存快照、只读配置、部署与回滚；概览区分最新保存版本和当前部署版本。Run 向已部署版本提交任务，展示真实执行历史，并链接配置与会话记录。关联版本的会话以只读说明和正在执行任务的取消按钮替代输入与模型控件。独立的 **Agent 工作台**保留旧式会话的不可变 Preset 创建入口；这些定义也会出现在资源 Registry 中。
