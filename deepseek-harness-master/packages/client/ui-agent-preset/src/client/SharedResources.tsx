...
import type { RegistryCatalog, ResourceInput, ResourceStatus, ResourceVersion, ResourceUsage, SharedResource } from '@deepseek-ai/dsh-agent-builder/types'
  ...
  return <main className={css.panel} aria-label={t('resourcesTitle')}>
    ...
    <div className={css.filters}><input aria-label={t('resourceSearch')} placeholder={t('resourceSearch')} value={filter} onChange={(event) => { setFilter(event.target.value) }} />
      <select aria-label={t('resourceKind')} value={kind} onChange={(event) => { setKind(event.target.value) }}>
        <option value="all">{t('all')}</option>{(['model', 'tool', 'skill'] as const).map(value => <option key={value} value={value}>{t(`resource_${value}`)}</option>)}</select>
    ...
    {draft === null && <div className={css.cards}>{rows.filter(row => (kind === 'all' || row.spec.kind === kind) && `${row.name} ${row.ownerTeamId}`.toLowerCase().includes(filter.toLowerCase())).map(row =>
      <article className={css.card} key={row.id} data-resource={row.id}><div className={css.cardTop}><span>{t(`resource_${row.spec.kind}`)}</span><span className={css.badge}>{t(row.status)}</span></div>
        ...
        <small>{row.ownerTeamId} · {row.versions.length === 0 ? t('resourceUnpublished') : `v${row.versions.at(-1)?.versionNumber}`}</small></article>)}</div>}
    {draft !== null && <section className={css.detail} aria-label={t('resourceEditor')}><h2>{selected?.name ?? t('resourceCreate')}</h2>
      <form onSubmit={(event) => { event.preventDefault()
        ...
        <fieldset className={css.form} disabled={busy}>
          ...
          }}>{(['model', 'tool', 'skill'] as const).map(value => <option key={value} value={value}>{t(`resource_${value}`)}</option>)}</select></label>
          ...
          {draft.spec.kind === 'tool' && <label>{t('toolOperation')}<select aria-label={t('toolOperation')} value={draft.spec.operation} onChange={(event) => { change({ spec: { kind: 'tool', operation: event.target.value } }) }}>
      ...
      <h3>{t('versions')}</h3>{selected.versions.slice().reverse().map(version => <details key={version.id}><summary>{`v${version.versionNumber}`} · {new Date(version.createdAt).toLocaleString()}</summary>
      ...
      <h3>{t('resourceUsage')}</h3>{usage.length === 0 ? <p>{t('resourceUnused')}</p> : usage.map((use, index) => <p key={index}>
        <a href={`#agents/${use.agentId}`}>{use.agentName}</a> · {use.versionNumber === null ? t('draft') : `v${use.versionNumber}`} {use.deployed && t('deployed')}</p>)}</>}
    </section>}
  </main>
}
