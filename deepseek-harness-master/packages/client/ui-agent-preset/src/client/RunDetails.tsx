...
export function runDuration(run: PlatformRun, now: number): number | null {
  ...
  const end = run.status === 'RUNNING' ? now : run.finishedAt === null ? null : Date.parse(run.finishedAt)
 ...
 */
export function RunDetails({ run, agentName, now, busy, cancel, openExecution, viewVersion, t }: Props) {
