...
import { resourceInputSchema, resourceStatusSchema } from './resource-schema.ts'
...
const actions: Record<string, WorkspaceAction> = {
  catalog: 'edit', agents: 'read', agent: 'edit', createAgent: 'edit', updateAgent: 'edit', archiveAgent: 'edit',
 ...
 */
export async function platformDispatch(builder: AgentBuilder, governance: Governance, actor: string, workspace: string,
  ...
  return governance.perform(actor, workspace, action, () => withPrincipal({ userId: actor, workspaceId: workspace }, async () => {
    if (action === 'admin' && governance.workspace(workspace).status !== 'active') throw new GovernanceError(403, 'Workspace is archived')
    switch (operation) {
      ...
      case 'start': return builder.runStart(workspace, ...z.tuple([text, z.string().min(1).max(32000), tokenSchema]).parse(args))
  ...
  }), ['createAgent', 'updateAgent', 'archiveAgent', 'saveVersion', 'deploy', 'createResource', 'updateResource', 'publishResource', 'resourceStatus', 'seedResources', 'resolve'].includes(operation) ? operation : undefined, typeof args[0] === 'string' ? args[0] : undefined)
}
