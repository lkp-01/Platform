...
export class AgentVersions {
  ...
  get(workspace: string, agentId: string, versionId: string): AgentVersion {
    ...
    if (version === undefined) throw new RegistryError('not-found', 'Version not found')
  ...
  async create(workspace: string, agentId: string, revision: number, token: string, note: string,
    validate: (draft: RegistryAgent) => Promise<LlmCallConfig | void>): Promise<AgentVersion> {
    ...
    return this.registry.exclusive(workspace, agentId, async () => {
      ...
      if (receipt !== undefined) {
        if (receipt.fingerprint !== fingerprint) throw new RegistryError('conflict', 'Version request token already used for different input')
      ...
      const snapshot = captureSnapshot(draft, resolved)
      const id = brandString<AgentVersionId>(`version-${createHash('sha256').update(JSON.stringify([workspace, agentId, token])).digest('hex').slice(0, 32)}`)
      const version: AgentVersion = { id, agentId: draft.id, platformWorkspaceId: workspace, sourceRevision: revision,
        versionNumber: (previous?.versions.length ?? 0) + 1, schemaVersion: 1, snapshot, configHash: snapshotHash(snapshot),
        changeNote: note, createdAt: new Date().toISOString(), createdBy: 'shared-host' }
      await table.put(agentId, { versions: [...(previous?.versions ?? []), version],
        receipts: { ...previous?.receipts, [token]: { fingerprint, versionId: id } } })
      return structuredClone(version)
    })
  }
}
