...
[Platform Runs](src/platform-runs.ts) persist admission and input before preparing a fixed version and creating its dedicated Harness Session. Every Run has one of PENDING, RUNNING, SUCCEEDED, FAILED or CANCELLED. Harness events persist start/end times, a bounded final-answer preview, output reference and structured error; list queries read these task records without scanning transcripts. Repeated admission tokens never submit twice. Cancellation prevents pending submission or requests the original Harness cancellation, keeping RUNNING until execution settles. Process interruption becomes FAILED with EXECUTION_INTERRUPTED and detected timing, without resubmission. Terminal facts are immutable; startup reconciles legacy or unfinished admissions against the original Session events. Storage read errors remain errors rather than invented execution failures. The Session and Preset authorization hooks preserve fixed configurations and ordinary Session behavior.
